...
public class Temporada {
	...
	public void anadirCapituloDespues(String nombreCapituloAnnadir, String nombreCapituloAnterior) throws SerieException{
		if(!this.capitulos.contains(nombreCapituloAnterior)) {
			this.capitulos.add(nombreCapituloAnnadir);
			throw new SerieException("no existe ep anterior");
		}
	...
	public String primerCapituloQueContieneEstaPalabara(String palabra) throws SerieException {
		Iterator<String> iteradorEpisodios = this.capitulos.iterator();
		String output = null;
		while(iteradorEpisodios.hasNext() && output == null) {
			String tmp = iteradorEpisodios.next();
			if(tmp.contains(palabra)) {
				output = tmp;
			}
		}
		
		if(output == null) {
			throw new SerieException("No hay episodio que contenga esa palabra");
		}
	...
	public double getNotaMedia() {
		...
		if (numeroOpiniones != 0) {
			notaMedia = ((double)sumaOpiniones)/numeroOpiniones;
		}
		return notaMedia;
	}
