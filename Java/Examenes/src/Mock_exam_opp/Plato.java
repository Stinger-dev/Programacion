...
public class Plato {
	...
	public String toString() {
		...
		resultado.append(String.format("El plato se llama %s",
					this.nombre));
		if(this.precio>0) {
			resultado.append(String.format(", tiene un precio sin iva de %s y de %s con iva incluido\n",this.precio, this.getPrecioVentaPublico()));
		}else {
		...
		if (this.recomendado.getNombre().equalsIgnoreCase("Sin recomendaciones de vino"))
			...
			resultado.append(String.format("El vino recomendado es %s y tiene una graduacion de %s", this.recomendado.getNombre(), this.recomendado.getGraduacion()));			
