...
public abstract class Producto {
	...
	public Producto(String nombre, String descripcion, Alergeno alergeno, double precioBase) {
		...
		this.codigoProducto = this.getClass().getSimpleName().concat(String.valueOf(++Restaurante.secuenciaGlobal)); // empieza
																														// por
																														// 1
	}
	...
	public String toString() {
		return String.format("%s: %s \n" + "Precio: %s \n" + "Presenta alergenos: %s", this.getClass().getSimpleName(),
				this.nombre, this.getPrecioProducto(),
				(this.alergeno == null) ? "No tiene alergenos" : "Si tiene alergenos");
	}
