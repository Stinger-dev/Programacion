...
public class GranAlmacen {
	...
	private int getMejorCaja() {
		...
		/**
		 * En caso de que no haya ninguna caja disponible, delvolvera un -1, en caso contrario, devolvera el numero de la caja con la menor cola y en caso de empate, la que tenga un numero mejor
		 * 
		 */
		int menor = Integer.MAX_VALUE;
		int resultado = -1;
		
		for (int i = 0; i < NUMERO_DE_CAJAS; i++) { //poniendo que sea estrictamente mayor y empezando desde los numeros mas bajos me aseguro de, en caso de empate, se quede la que tiene el numero menor
	...
	public String atenderCliente(int nCaja) throws CajaException {
		String resultado;
		if(nCaja >= 0 && nCaja < NUMERO_DE_CAJAS && this.cajas[nCaja].isAbierto() && this.cajas[nCaja].getTamagnoCola() != 0) {
