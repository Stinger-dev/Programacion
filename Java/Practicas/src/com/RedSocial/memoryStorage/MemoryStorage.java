...
public class MemoryStorage {
	...
	public void addPublicacion(String txt, String login, String tema) throws MemoryStorageExeption { // Añadir Post
		if (txt != null && !txt.trim().isEmpty() && login != null && !login.trim().isEmpty() && tema != null
				&& !tema.trim().isEmpty()) {
			int posicion = this.posicionUsuario(login);
			if (posicion != -1) {
				Usuario user = this.usuarios[posicion];
	...
	public void addPublicacion(String txt, String login, int estellas) throws MemoryStorageExeption { // Añadir
																										// recomendacion
